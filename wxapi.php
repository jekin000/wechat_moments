...
class wechatCallbackapiTest
{
    ...
    private function parse_keyword($username,$key)
    {
            ...
            $cmdarr = explode(" ",$key,2);
            $cmdnostr  = $cmdarr[0];
            if ($cmdnostr == '000')
                return $heartstone->showMenu();
            else if ($cmdnostr == '001' && count($cmdarr)>1)
                return $heartstone->showCreateDeckResult($username,$cmdarr[1]);
            ...
            else if ($cmdnostr == '005')
                return $heartstone->showListDeck($username,'by-rate');
            else if ($cmdnostr == '006' && count($cmdarr)>1)
                return $heartstone->showDrawCards($username,$cmdarr[1]);
            else if ($cmdnostr == '799')
                return $heartstone->dbgShowallkeys($username);
            else if ($cmdnostr == '800' && count($cmdarr)>1)
                return $heartstone->dbgDelAnyDeck($username,$cmdarr[1]);
